...
async def _choose_task_to_do(callback_query: CallbackQuery):
    ...
    await _view_task(callback_query.message, task, 'to-do-choose', response)
...
async def _choose_task_completed(callback_query: CallbackQuery):
    ...
    response = ''
    await callback_query.message.delete()
    await _view_task(callback_query.message, task, 'completed-choose', response)
...
async def _choose_task_edit(callback_query: CallbackQuery):
    ...
    response = ''
    await callback_query.message.delete()
    await _view_task(callback_query.message, task, 'edit', response)
